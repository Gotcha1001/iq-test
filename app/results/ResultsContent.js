...
// Normative data and calculation functions
...
const calculateTypeScores = (score, timeTaken, answeredQuestions) => {
  ...
  types.forEach((type) => {
    ...
    const zScore =
      typeMaxScore > 0
        ? (typeScore - (typeMaxScore * NORMATIVE_DATA.meanScore) / 40) /
          ((typeMaxScore * NORMATIVE_DATA.sdScore) / 40)
        : 0;
    const zKey = Math.round(zScore * 2) / 2;
    const percentile = PERCENTILE_TABLE[zKey.toFixed(1)] || 50;
    typeScores[type] = {
...
export default function ResultsContent() {
  ...
  const validTimeTaken = timeTaken.slice(0, totalQuestions);
  ...
  const correctAnswers = validAnsweredQuestions.filter(
    (q, i) =>
      q.selectedOptionIndex >= 0 &&
      q.correct === q.options[q.selectedOptionIndex]
  ).length;
  const totalTime = validTimeTaken.reduce((sum, t) => sum + t, 0);

  const handleRetakeAssessment = () => {
    console.log("Retake Assessment clicked, navigating to: /");
    window.location.href = "/"; // Use window.location.href for full navigation
  };
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(typeScores).map(([type, score]) => (
              <div key={type} className="bg-white/5 rounded-lg p-4">
                <h3 className="text-lg font-medium mb-2 capitalize text-purple-300">
                  {type}
                </h3>
